...
def get_solution(out_file):
    ...
    for line in open(out_file, "r"):
        ...
        if s:
            ...
            sol.insert(
                0, "0"
            )  # Adds a 0 at the begginig to make variable 'i' at potition 'i'
            try:
...
def check_solution(solution, benchmark_file):
    ...
    for line in instance:
        if line[0] in ["c", "p"]:  # Pass comments and program line
            continue
...
if __name__ == "__main__":
    ...
    for bf in benchmark_files:
        ...
        with open(out_file, "w") as output:
            subprocess.run(
                ["time", "-p", "./%s" % limits_file, solver, bf],
                stdout=output,
        ...
        if correct:  # The solution is correct
            # Get Time
            time = get_time(out_file)
            if time is None:  # This should not happend
                time = timeout * inc_to
                sys.stdout.write("Time not found! time = %.2f\n" % time)
            else:
                time = float(time)
                sys.stdout.write("OK! time = %.2f\n" % time)
        elif correct is None:  # There is no solution
            time = timeout * inc_to
            sys.stdout.write("No solution found! time = %i\n" % time)
        elif not correct:  # There is a bug in the solution
